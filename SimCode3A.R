...
true = read.csv('/Users/jrigdon/Box Sync/Rigdon/Sanjay/HTE/DATA3A.csv', header=TRUE, colClasses=c("numeric", "factor", rep("numeric", 4), rep("factor", 2), "numeric", rep("factor", 4), "character"))
...
##Look at ATE
r1 = mktabASD(data=true, var.names=covs, ind.cat=ind.cat, group.name="Z", miss='always', digit=2)
...
table(true$Y, true$Z)
mean(true$Y[true$Z==1])-mean(true$Y[true$Z==0]) #-0.003 (close to true ATE of -0.0008)
prop.test(x=c(193, 190), n=c(3000, 3000))
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/LR_3.pdf")
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/GRF_3.pdf")
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/RF_3.pdf")
plot(fitRF, type="simple") #2 terminal nodes
dev.off()

pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/mCART_3.pdf")
...
length(table(true$nodeLR1)) #442
length(table(true$nodeRF)) #3
length(table(true$nodeGRF1)) #46
