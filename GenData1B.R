...
##HTEs in women <=65 ~ Multinom(0.6, 0.2, 0.2) [ATE -0.4, Y1=0.4, Y0=0.8]
summary(true$age)
true$delta[true$age<=65] = rmulti(c(0.6, 0.2, 0.2), length(true$delta[true$age<=65]))-2
table(true$delta[true$age<=65])
mean(true$delta[true$age<=65])

##And >65 ~ Multinom(0.4, 0.2, 0.4) [ATE 0, Y1=0.6, Y0=0.6]
true$delta[true$age>65] = rmulti(c(0.4, 0.2, 0.4), length(true$delta[true$age>65]))-2
table(true$delta[true$age>65])
mean(true$delta[true$age>65])

##Fill in potential outcomes
true$Y1 = NA
true$Y0 = NA
...
##Look by age group
##<=65
mean(true$Y1[true$age<=65]) #~0.4
mean(true$Y0[true$age<=65]) #~0.8

##>65
mean(true$Y1[true$age>65]) #~0.6
mean(true$Y0[true$age>65]) #~0.6
...
##Save locally
write.csv(true, "/Users/jrigdon/Box Sync/Rigdon/Sanjay/HTE/DATA1B.csv", row.names=FALSE)
