...
##Look at ATE
r1 = mktabASD(data=true, var.names=c('black', 'eGFR', 'age', 'sex'), ind.cat=c(1, 0, 0, 1), group.name="Z", miss='always', digit=1)
...
te1 = c(te1, round(jr$estimate[1]-jr$estimate[2], 2)) #treatment effect (jr$conf.int[1] is conf int)
...
tt = table(dta$Y, dta$Z)
jr = prop.test(x=c(tt[2, 2], tt[2, 1]), n=c(apply(tt, 2, sum)[2:1]))
te2 = c(te2, round(jr$estimate[1]-jr$estimate[2], 2)) #treatment effect (jr$conf.int[1] is conf int)
...
tt = table(dta$Y, dta$Z)
jr = prop.test(x=c(tt[2, 2], tt[2, 1]), n=c(apply(tt, 2, sum)[2:1]))
te3 = c(te3, round(jr$estimate[1]-jr$estimate[2], 2)) #treatment effect (jr$conf.int[1] is conf int)
...
tt = table(dta$Y, dta$Z)
jr = prop.test(x=c(tt[2, 2], tt[2, 1]), n=c(apply(tt, 2, sum)[2:1]))
te4 = c(te4, round(jr$estimate[1]-jr$estimate[2], 2)) #treatment effect (jr$conf.int[1] is conf int)
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/Plot_TE_ASD_v2.pdf")
plot(te1, asd1, xlim=c(-1, 1), ylim=c(0, 1.6), col=1, pch=19, xlab = "Estimated treatment effect in subgroup", ylab="Max ASD in subgroup")
points(te2, asd2, col=1, pch=4)
points(te3, asd3, col=1, pch=0)
points(te4, asd4, col=1, pch=2)
abline(h=0.2, lty=2)
legend('topleft', legend=c('mCART', 'LR', 'RF', 'GRF'), col=c(1, 1, 1, 1), pch=c(19, 4, 0, 2), bty='n', inset=0.03)
dev.off()

##Save tables
word.doc(obj.list=list(tt1, tt2, tt3, tt4), obj.title=c('Table 1: mCART', 'Table 2: LR', 'Table 3: RF', 'Table 4: GRF'), dest='/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/Tab_HTE_v5.docx', ftype='Arial', col.odd='white')
