...
true = read.csv('/Users/jrigdon/Box Sync/Rigdon/Sanjay/HTE/DATA1A.csv', header=TRUE, colClasses=c("numeric", rep("factor", 8), "character"))
...
##Look at ATE
r1 = mktabASD(data=true, var.names=c('age', 'stage4', 'site', 'prevTrt', 'ecog', 'diseaseFree'), ind.cat=c(0, 1, 1, 1, 1, 1), group.name="Z", miss='always', digit=1)
...
prop.test(x=c(46, 70), n=c(100, 100))
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/LR_1A.pdf")
plot(fitLR1, type="simple") #2 terminal nodes
dev.off()
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/GRF_1A.pdf")
plot(fitGRF1, type="simple") #12 terminal nodes
dev.off()
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/RF_1A.pdf")
plot(fitRF, type="simple") #33 terminal nodes
dev.off()

pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/mCART_1A.pdf")
...
length(table(true$nodeLR1)) #2
#length(table(true$nodeLR2)) #1
length(table(true$nodeRF)) #5
length(table(true$nodeGRF1)) #5
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/Plot_TE_ASD_1A.pdf")
plot(asd1, b1, xlim=c(0, 1), ylim=c(0, 0.2), col=1, pch=19, xlab="Max of ASD across all covariates within subgroup", ylab="Absolute value of bias of estimated subgroup treatment effect", )
...
##Save tables
word.doc(obj.list=list(tt1, tt2, tt3, tt4), obj.title=c('Table 1: mCART', 'Table 2: LR', 'Table 3: RF', 'Table 4: GRF'), dest='/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/Tab_HTE_1A.docx', ftype='Arial', col.odd='white')
