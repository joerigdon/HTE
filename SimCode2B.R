...
true = read.csv('/Users/jrigdon/Box Sync/Rigdon/Sanjay/HTE/DATA2B_v7.csv', header=TRUE, colClasses=c("numeric", "factor", rep("numeric", 4), rep("factor", 2), "numeric", rep("factor", 4), "character"))
...
##Look at ATE
r1 = mktabASD(data=true, var.names=covs, ind.cat=ind.cat, group.name="Z", miss='always', digit=1)
...
table(true$Y, true$Z)
mean(true$Y[true$Z==1])-mean(true$Y[true$Z==0]) #-0.015 (close to true ATE of -0.016)
prop.test(x=c(160, 203), n=c(3000, 3000))

#Now use logistic regression w/ interaction term,
lr = glm(Y ~ (age + black + sbp + dbp + scr + eGFR + statin + aspirin + fram + smok)*Z , family='binomial', data=true)
lr.aic = step(lr, trace = F, direction=c("backward"))
df1 = true[, names(true) %in% covs]
df1$Z = 1
p1 = predict(lr.aic, df1, type='response')
df0 = true[, names(true) %in% covs]
df0$Z = 0
...
library(grf)
true2 = true
true3 = apply(as.matrix(true2[, names(true2) %in% covs]), 2, function(x) as.numeric(x))
set.seed(42)
...
#Regular RF
gg = true[, names(true) %in% c(covs, 'Z', 'Y')]
gg$Y = factor(gg$Y)
library(randomForest)
set.seed(3)
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/LR_2B.pdf")
plot(fitLR1, type="simple") #448 terminal nodes
dev.off()
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/GRF_2B.pdf")
plot(fitGRF1, type="simple") #24 terminal nodes
dev.off()
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/RF_2B.pdf")
plot(fitRF, type="simple") #2 terminal nodes
dev.off()

pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/mCART_2B.pdf")
plot(jj$fit) #How to control this plot??
dev.off()
...
length(table(true$nodeLR1)) #436
length(table(true$nodeRF)) #3
length(table(true$nodeGRF1)) #37
length(table(true$nodemC)) #4
...
if (dim(t1m)[2]==2) {t1m=cbind(t1m, NA)}
...
pdf("/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/Plot_TE_ASD_2B.pdf")
plot(asd1, b1, xlim=c(0, 4), ylim=c(0, 0.3), col=1, pch=19, xlab="Max of ASD across all covariates within subgroup", ylab="Absolute value of bias of estimated subgroup treatment effect", )
...
##Save tables
word.doc(obj.list=list(tt1, tt2, tt3, tt4), obj.title=c('Table 1: mCART', 'Table 2: LR', 'Table 3: RF', 'Table 4: GRF'), dest='/Users/jrigdon/Box sync/Rigdon/Sanjay/HTE/Tab_HTE_2B.docx', ftype='Arial', col.odd='white')
